package com.bbmovie.homepagerecommendations.security;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(HttpMethod.GET, "/api/homepage/v1/trending")
                        .permitAll()
    ...
    public JwtDecoder jwtDecoder() {
        if (isDevProfile()) {
            log.warn("[Security] [profile: {}] Using hardcoded DEV JWK for JWT validation!", activeProfile);
