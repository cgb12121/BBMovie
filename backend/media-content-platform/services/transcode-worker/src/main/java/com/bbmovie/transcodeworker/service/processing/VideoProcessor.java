...
public class VideoProcessor implements MediaProcessor {
    ...
    public ProcessingResult process(ExecuteTask task, Path inputFile, Path outputDir) {
        ...
        try {
            ...
            List<String> availableResolutions = resolutions.stream()
                    .map(VideoTranscoderService.VideoResolution::filename)
                    .filter(name -> name != null && !name.isBlank())
                    .sorted(Comparator.comparingInt(this::resolutionHeight))
                    .toList();

            log.info("Video processing completed: {} (video duration: {}s, resolutions={})",
                    taskId, probe.duration(), availableResolutions);
