...
Detailed proxy flow and dependencies are documented in `PROXY_MEDIA_STREAM.md`.

**Direct (non-proxy) streaming** — pre-signed MinIO URLs, parallel route prefix `/api/stream/direct/**` — is documented in `DIRECT_MEDIA_STREAM.md`.
...
### Stream Architecture
- **Master Playlist (proxy)**: `/api/stream/{movieId}/master.m3u8`
- **Resolution Playlists (proxy)**: `/api/stream/{movieId}/{resolution}/playlist.m3u8`
- **Direct (pre-signed)**: `/api/stream/direct/...` — see `DIRECT_MEDIA_STREAM.md`
- **Encryption Keys (proxy)**: `/api/stream/keys/{movieId}/{resolution}/{keyFile}`
