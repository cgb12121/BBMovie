...
public class SessionServiceImpl implements SessionService {
    ...
    public Mono<CursorPageResponse<ChatSessionResponse>> activeSessionsWithCursor(UUID userId, String cursor, int size) {
        Flux<ChatSession> sessionFlux = (cursor == null || cursor.isBlank())
                ? sessionRepository.findActiveSessions(userId, size + 1)
                : sessionRepository.findActiveSessionsWithCursor(userId, Instant.parse(cursor), size + 1);

        return sessionFlux.collectList()
                .map(sessions -> CursorPageHelper.toPage(sessions, size,
                        s -> s.getCreatedAt().toString(),
                        s -> new ChatSessionResponse(s.getId(), s.getName(), s.getCreatedAt(), s.isArchived())));
    }
    ...
    public Mono<CursorPageResponse<ChatSessionResponse>> listArchivedSessions(UUID userId, String cursor, int size) {
        Flux<ChatSession> sessionFlux = (cursor == null || cursor.isBlank())
                ? sessionRepository.findArchivedSessions(userId, size + 1)
                : sessionRepository.findArchivedSessionsWithCursor(userId, Instant.parse(cursor), size + 1);
    ...
    public Mono<Void> deleteArchivedSessions(List<UUID> sessionIds, UUID userId) {
        // Bulk ownership check: filter to only sessions owned by this user before deleting.
        return sessionRepository.findAllById(sessionIds)
                .filter(session -> session.getUserId().equals(userId))
                .map(ChatSession::getId)
                .collectList()
                .flatMap(ownedIds -> sessionRepository.deleteAllById(ownedIds));
    }
}
