...
public class SubscriptionEventConsumer {
    ...
    @Autowired
    public SubscriptionEventConsumer(ObjectMapper objectMapper, EmailServiceFactory emailServiceFactory) {
    ...
    public void onNatsConnection(NatsConnectionEvent event) {
        if (event.type() == ConnectionListener.Events.CONNECTED || event.type() == ConnectionListener.Events.RECONNECTED) {
            log.info("NATS connected/reconnected, (re)subscribing to subscription events…");
            setupSubscriptionServiceEventSubscriptions(event.connection());
        }
    }

    public void setupSubscriptionServiceEventSubscriptions(Connection nats) {
